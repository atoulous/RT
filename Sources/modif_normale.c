...
double noise(double x, double y, double z) {
	...
	t_perlin myPerlin;
	myPerlin.p = (int *)malloc(sizeof(int) * 1000);
	perlin(myPerlin.p);
	...
	double u = fade(x),                                // COMPUTE FADE CURVES
		   ...
		   w = fade(z);
	int A = myPerlin.p[X  ]+Y; int AA = myPerlin.p[A]+Z; int AB = myPerlin.p[A+1]+Z;     // HASH COORDINATES OF
	int B = myPerlin.p[X+1]+Y; int BA = myPerlin.p[B]+Z; int BB = myPerlin.p[B+1]+Z;      // THE 8 CUBE CORNERS,
