...
/*   Updated: 2016/12/15 15:19:34 by jubarbie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		find_dist(t_object *obj, t_ray *ray, double *sol)
{
	...
	if (max < ray->dist)
	{
		...
		if (index == 0 || index == 1)
			...
			ray->norm = unit_v3d(sub_v3d(O_P2, obj->pos));
		else if (index == 3)
			ray->norm = unit_v3d(sub_v3d(O_POS, O_P2));
	}
}
...
static void		find_solutions(t_object *obj, t_ray *ray, t_v3d abc)
{
	...
	if ((det = ft_solve_quadratic(abc.x, abc.y, abc.z, sol)) >= 0)
	{
		...
		sol[2] = caps(ray, O_R1, n, p2);
		sol[3] = caps(ray, O_R1, smul_v3d(n, -1), O_POS);
